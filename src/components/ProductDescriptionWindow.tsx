...
export function ProductDescriptionWindow() {
  ...
  // 获取当前选中的产品数据
  // 优先从 activeTabId 获取，如果没有则使用 selectedProduct
  const selectedProductData = useMemo(() => {
    // 如果有活动标签页，优先使用标签页中的产品
    if (activeTabId) {
      ...
      <div style={{ 
        ...
        <Spin spinning={goodsInfoLoading}>
          {isEditing ? (
            ...
            onClick={() => setIsEditing(true)}
            >
              {goodsInfo || <Text type="secondary">点击编辑产品简介...</Text>}
            </div>
          )}
        </Spin>
